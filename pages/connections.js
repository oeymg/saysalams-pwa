    ...
    <Layout>
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '2rem 1rem' }}>
        <header className="connections-header" style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: '1rem' }}>
          <h1 style={{ color: 'var(--accent)', margin: 0 }}>My Connections</h1>
          <Link href="/profile" style={{ color: 'var(--accent)', textDecoration: 'underline' }}>View profile</Link>
        </header>
            ...
            <div style={{ display: 'grid', gap: '1rem' }}>
              <section style={secStyle}>
                ...
                {accepted.length === 0 ? (
                  ...
                  <ul style={list}>
                    {accepted.map((c) => (
                      <li key={c.id} style={pill('var(--surface-2)', 'var(--border-soft)')}>
                        <div>
                          ...
                          {Array.isArray(c.other?.interests) && c.other.interests.length > 0 && (
                            <div style={{ marginTop: 6 }}>
                              {c.other.interests.slice(0, 3).map((t) => (
                                <span key={t} style={{ background: 'var(--warm)', color: 'var(--accent)', padding: '0.15rem 0.5rem', borderRadius: 6, fontSize: '0.75rem', marginRight: 6 }}>{t}</span>
                              ))}
                            </div>
                          )}
              ...
              <section style={secStyle}>
                ...
                {incoming.length === 0 ? (
                  ...
                  <ul style={list}>
                    {incoming.map((c) => (
                      <li key={c.id} className="conn-item" style={{ ...pill('var(--surface-2)', 'var(--border-soft)') }}>
                        <div>
                          <strong style={{ cursor:'pointer' }} onClick={() => openQuick(c.other)}>{c.other?.name || 'User'}</strong>
                          <div style={{ color: 'var(--warning-contrast)', fontSize: '0.9rem' }}>Pending</div>
                          ...
                          {Array.isArray(c.other?.interests) && c.other.interests.length > 0 && (
                            <div style={{ marginTop: 6 }}>
                              {c.other.interests.slice(0, 3).map((t) => (
                                <span key={t} style={{ background: 'var(--surface-2)', color: 'var(--warning-contrast)', padding: '0.15rem 0.5rem', borderRadius: 6, fontSize: '0.75rem', marginRight: 6 }}>{t}</span>
                              ))}
                            </div>
                          )}
                        </div>
                        <div className="conn-actions">
                          <button onClick={() => actOn(c.id, 'accept')} style={btn('var(--success)')}>Accept</button>
                          <button onClick={() => actOn(c.id, 'decline')} style={btn('var(--danger)')}>Decline</button>
                        </div>
                      </li>
                    ))}
                  </ul>
                  ...
                  <ul style={list}>
                    {sent.map((c) => (
                      <li key={c.id} className="conn-item" style={{ ...pill('var(--surface-2)', 'var(--border-soft)') }}>
                        <div>
                          <strong style={{ cursor:'pointer' }} onClick={() => openQuick(c.other)}>{c.other?.name || 'User'}</strong>
                          <div style={{ color: 'var(--muted-2)', fontSize: '0.9rem' }}>Awaiting response</div>
                          <div style={{ color: 'var(--muted-2)', fontSize: '0.9rem' }}>
                            {[c.other?.postcode, c.other?.location].filter(Boolean).join(' · ')}
                          </div>
                          {Array.isArray(c.other?.interests) && c.other.interests.length > 0 && (
                        ...
                        <div className="conn-actions">
                          <button onClick={() => actOn(c.id, 'withdraw')} style={btn('var(--danger)')}>Withdraw</button>
                        </div>
                      </li>
                    ))}
                  </ul>
              ...
              {Array.isArray(recs) && recs.length > 0 ? (
                <ul style={list}>
                  {recs.map((r) => {
                    ...
                    return (
                      <li key={r.user.record_id} className="conn-item" style={{ ...pill('var(--surface-2)', 'var(--border-soft)') }}>
                        <div>
                        ...
                        {edge ? (
                          mine ? (
                            <div className="conn-actions">
                              <span className="chip" style={{ background:'var(--warm)', color:'var(--accent)' }}>Pending</span>
                              <button onClick={() => actOn(edge.id, 'withdraw')} style={btn('var(--danger)')}>Withdraw</button>
                            </div>
                          ) : (
                            <Link href="/connections" className="chip" style={{ textDecoration:'none', background:'var(--warm)', color:'var(--accent)' }}>Review request</Link>
                          )
                        ) : (
                          <button onClick={() => request(r.user.record_id)} style={btn('var(--accent)')}>Say Salams</button>
                        )}
                      </li>
                    );
                  })}
                ...
                <>
                  <h3 style={{ color: 'var(--accent)', marginTop: '0.25rem' }}>Discover People</h3>
                  {discover.length === 0 ? (
                    ...
                    <ul style={list}>
                      {discover.map((u) => {
                        ...
                        const mine = edge && edge.requester === me?.record_id;
                        return (
                          <li key={u.record_id} className="conn-item" style={{ ...pill('var(--surface-2)', 'var(--border-soft)') }}>
                            <div>
                            ...
                            {edge ? (
                              ...
                              <button onClick={() => request(u.record_id)} style={btn('var(--accent)')}>Say Salams</button>
                            )}
                          </li>
                        );
                      })}
        ...
        {toast && (
          <div onClick={() => setToast(null)} style={{ position: 'fixed', right: 16, bottom: 16, zIndex: 1000, background: toast.type === 'error' ? 'var(--danger)' : 'var(--accent)', color: '#fff', padding: '0.75rem 1rem', borderRadius: 8, boxShadow: '0 6px 20px rgba(0,0,0,0.15)', cursor: 'pointer' }}>{toast.message}</div>
        )}
      </div>
      {quick && (
        <QuickProfile
      ...
      <div onClick={(e)=>e.stopPropagation()} style={{ background:'var(--surface)', borderRadius:12, padding:'1rem', maxWidth:420, width:'100%', boxShadow:'0 10px 28px rgba(0,0,0,0.25)' }}>
        ...
        <div style={{ display:'flex', justifyContent:'center', marginBottom:8 }}>
          {user?.image_url ? (
            ...
            <div style={{ width:96, height:96, borderRadius:'9999px', background:'var(--surface-2)', color:'var(--accent)', display:'grid', placeItems:'center', fontWeight:800, fontSize:'1.4rem' }}>
              {initials(user?.name)}
            </div>
          )}
        </div>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
          <h3 style={{ margin:0, color:'var(--accent)' }}>{user?.name || 'User'}</h3>
        ...
        {Array.isArray(user?.interests) && user.interests.length > 0 && (
          ...
          <Link href={`/profile/${encodeURIComponent(user.record_id)}`} style={{ color:'var(--accent)', fontWeight:800, textDecoration:'none' }}>View Profile</Link>
        ...
        {showPhoto && (
          <div onClick={()=>setShowPhoto(false)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.75)', zIndex:1100, display:'flex', alignItems:'center', justifyContent:'center', padding:'1rem' }}>
            <div onClick={(e)=>e.stopPropagation()} style={{ position:'relative', width:'90vw', height:'85vh', maxWidth:'900px', maxHeight:'85vh' }}>
              ...
              {user?.image_url ? (
                <Image
                  ...
                  fill
                  sizes="90vw"
                  style={{ objectFit:'contain', borderRadius:12, display:'block' }}
                />
              ) : null}
            </div>
          </div>
